...
class Verify {
    ...
    public function check($code, $id = '') {
        ...
        // 验证码不能为空
        file_put_contents('/opt/ok/openapi.shangjiadao.com/Application/Runtime/Logs/Gw/cache.log', "get code when check _sessions1: ".json_encode($_SESSION)."\n", FILE_APPEND);

        $secode = json_decode(session($key), true);

        file_put_contents('/opt/ok/openapi.shangjiadao.com/Application/Runtime/Logs/Gw/cache.log', "get code when check session_id: ".session_id()."\n", FILE_APPEND);
        file_put_contents('/opt/ok/openapi.shangjiadao.com/Application/Runtime/Logs/Gw/cache.log', "get code when check _sessions2: ".json_encode($_SESSION)."\n", FILE_APPEND);
        file_put_contents('/opt/ok/openapi.shangjiadao.com/Application/Runtime/Logs/Gw/cache.log', "get code when check: ".json_encode($secode)."\n", FILE_APPEND);
     ...
     * @auth qfb
     */
    public function entry2base64($id = '') {
        // 图片宽(px)
        $this->imageW || $this->imageW = $this->length*$this->fontSize*1.5 + $this->length*$this->fontSize/2; 
        // 图片高(px)
        $this->imageH || $this->imageH = $this->fontSize * 2.5;
        ...
        $secode['verify_code'] = $code; // 把校验码保存到session
        $secode['verify_time'] = NOW_TIME;  // 验证码创建时间
        session($key.$id, json_encode($secode));

        // file_put_contents('/opt/ok/openapi.shangjiadao.com/Application/Runtime/Logs/Gw/cache.log', "get code after setted: ".(session($key.$id))."\n", FILE_APPEND);
        // file_put_contents('/opt/ok/openapi.shangjiadao.com/Application/Runtime/Logs/Gw/cache.log', "get code after setted session_id: ".$session_id."\n", FILE_APPEND);
        // file_put_contents('/opt/ok/openapi.shangjiadao.com/Application/Runtime/Logs/Gw/cache.log', "get code after setted _sessions: ".json_encode($_SESSION)."\n", FILE_APPEND);
